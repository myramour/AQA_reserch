...
public class Product_Test extends BaseTest {
    ...
    public void productTest( @Optional("0") String steps){
        if (steps.equals("0")) get(LoginSteps.class).login(properties.getProperty("username"), properties.getProperty("password")); //если параметр степ0 в хмл, то запустит со степами. если памрерт 1 то без степов(цепочка классов)(в одном потоке со сьепами в другом с ))
